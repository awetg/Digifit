...
class DistanceFragment : GraphBaseFragment() {
    ...
    override fun onCreateView(
        ...
        view.dail_count_unit_txt.text = getString(R.string.DistanceUnit)
        view.dail_count_description_txt.text = getString(R.string.txtDistanceWalked)
        ...
        stepViewModel.getStepCountBetweenTimeLive(todayStartAndEnd.first.timeInMillis, todayStartAndEnd.second.timeInMillis).observe(this, Observer {
            view.daily_count_txt.text = DecimalFormat(".##").format(CalorieAndDistanceCalculator.calculateDistanceForSteps(it) / 100)
        })
        ...
        launch {
            graphUtil.setDayGraph(
                getStepCountList(TimeUtil.getStartAndEndOfToday(), TimeUtil.secondsInHour, "%H")
                    ?.map { StepCount(CalorieAndDistanceCalculator.calculateDistanceForSteps(it.count).toInt(), it.intervalFormat) }
            )
        }
